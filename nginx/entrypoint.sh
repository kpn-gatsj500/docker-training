...
sed -E 's/(\$)([a-zA-Z_]+)/###ENVSUBSTNOTOUCH###\2/g' /etc/nginx/conf.d/nginx.conf > /tmp/temp_step1.conf
...
cp /tmp/temp_step3.conf /etc/nginx/conf.d/nginx.conf
echo "Final result moved to /etc/nginx/conf.d/nginx.conf"
